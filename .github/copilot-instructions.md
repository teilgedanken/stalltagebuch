...
## Build & Workflow
- nutze IMMER `./build_android.sh --release` um zu testen. das Script build_android.sh patcht Gradle, kopiert Manifest/Activity/Res und setzt Java 17.
- The User will do the building and testing on an Android Emulator. 
- Nutze NIE `dx build`;
- Nutze NIE `cargo check`;
- Nutze NIE `cargo test` - in fact as there are no tests implemented do not run any form of unit tests;
